...
public class Main_17069 {
	...
	public static void main(String[] args) throws Exception {
		...
		if (map[n + 1][n + 1] == 0 && map[n][n + 1] == 0 && map[n - 1][n + 1] == 0) {
			dp[1][n + 1][n + 1] = dfs(n + 1, n + 1, 2);
		} else {
			dp[1][n + 1][n + 1] = 0;
		}

		if (map[n + 1][n + 1] == 0 && map[n + 1][n] == 0 && map[n + 1][n - 1] == 0) {
			dp[0][n + 1][n + 1] = dfs(n + 1, n + 1, 1);
		} else {
			dp[0][n + 1][n + 1] = 0;
		}

		if (map[n + 1][n + 1] == 0 && map[n][n] == 0 && map[n][n - 1] == 0 && map[n - 1][n] == 0 && map[n - 1][n - 1] == 0 && map[n + 1][n] == 0 && map[n][n + 1] == 0) {
			dp[2][n + 1][n + 1] = dfs(n + 1, n + 1, 3);
		} else {
			dp[2][n + 1][n + 1] = 0;
		}
	...
	private static long dfs(int y, int x, int type) {
		...
		if (type == 1) {
			...
			} else if (map[y - 1][x - 1] == 0 && map[y - 2][x - 1] == 0 && map[y - 1][x - 2] == 0
					&& map[y - 2][x - 2] == 1) {
				if (dp[0][y - 1][x - 1] == -1) {
					dp[0][y - 1][x - 1] = dfs(y - 1, x - 1, 1);
				}
